...
At Xata, we have been working on a new Postgres project, which doesn't really have a name yet, so let's call it _"Xata's take at distributed Postgres"_. It is only in its very early stages, however we'll be open sourcing it soon and we're planning to [build it in public](https://mailchi.mp/xata/2zoy27tx2e). It will be somewhat similar to Citus, yet with some different choices, especially around usage and DX, based on what we learned running Xata for the past couple of years.
