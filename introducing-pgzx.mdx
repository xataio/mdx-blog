...
If you looked carefully at the code above, you might have noticed that it contains a bug. It checks for the `target_char` to not have more than 1 character, but it doesn't check if it has 0 characters. Later, the code accesses `target_char[0]`, so if the string is the empty string, there will be a buffer overflow error. We left that bug intentionally in so you can see what happens when bugs like that happen in the extension.
