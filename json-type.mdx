...
```jsonc
// POST https://{workspace}.{region}.xata.sh/db/{db}:{branch}/{tables}/{table_name}/columns
{
  "name": "details",
  "type": "json"
}
...
- A t-shirt with a size and a color
...
```jsonc
    // POST https://{workspace}.{region}.xata.sh/db/{db}:{branch}/{tables}/{table_name}/data
    {
        "name": "Xata xwag T-shirt",
    ...
    {
        "name": "Meditations",
        "details": "{\"author\": \"Marcus Aurelius\", \"isbn\": \"978-0140449334\"}"
    }
...
Basic JSON support is one more step in that direction along with the previously released Files.
