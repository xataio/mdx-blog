...
Schemafull data models are great. In Xata we are convinced that they are the best way to model your data in most cases,
...
JSON documents as fields in a relational data store are a great way have the best of both worlds.

Many of our users have been asking for this feature, and we are happy to announce that it's finally here.
Basic support for JSON column types has been added and it will bring many benefits:
...
Let's get into the details and provide a few examples on how you can start using JSON documents in Xata today.
...
We could use a JSON column to store the product details. Let's do this and add a `details` field to our table:
