...
- Programtically create Stripe checkout sessions
...
- [Enabling Server Side Rendering in Astro](#enabling-server-side-rendering-in-astro)
- [Setting up a Xata Database](#setting-up-a-xata-database)
...
- [Authenticating users with Lucia Auth and Xata](#authenticating-users-with-lucia-auth-and-xata)
  - [Building Xata Adapter for Lucia Auth](#building-xata-adapter-for-lucia-auth)
  ...
  - [Building Authentication Routes with Lucia Auth](#building-authentication-routes-with-lucia-auth)
...
For styling the app, you will use Tailwind CSS. Install and set up Tailwind at the root of our project's directory by running:
...
To create checkouts using Stripe API dynamically, you would need to server-side render your Astro application. Enable server side rendering in your Astro project by executing the following command in your terminal:
...
![Create Xata database](building-protected-and-paywall/create-database.png)

### Install the Xata CLI

Obtain the Xata initialization command by clicking on a table (say, `user`), and then on `Get code snippet` button.

![View User Table](building-protected-and-paywall/user-table.png)

A modal titled `Code snippets` is presented which contains set of commands to integrate Xata into your project, locally.

![Code Snippet for Xata CLI](building-protected-and-paywall/code-snippet.png)

For interacting with Xata database using their TypeScript SDK, install the Xata CLI globally by executing the following command in your terminal:
...
Then, link the Xata project to your Astro application by executing the following command in your terminal window:
...
![Setup Astro project with Xata CLI](building-protected-and-paywall/cli.png)

Now, let’s move on to the creating API endpoints that create checkouts dynamically using Stripe.
...
- Imports the Stripe class from it's SDK, `APIContext` type by Astro and `getXataClient` to access the in-memory Xata instance.
...
Let's move on to authenticating users with Xata database and Lucia library.
...
Now, let's move on to creating a Xata adapter for Lucia library.

### Building Xata Adapter for Lucia Auth
...
The code above imports the `getXataClient` utility to retrieve the in-memory Xata instance. It then imports the relevant types from `lucia` module. Append the following code:
...
- `SessionSchema`: A type created with `RegisteredDatabaseSessionAttributes` type definition alongwith `id` attribute as a string, `user_id` attribute as a string and `expires_at` attribute as a Date.
...
The code above defines a class XataAdapter based on Lucia's Adapter class. Then, it sets a `controller` private class variable equivalent to Xata instance obtained from `getXataClient` method. Define a method to delete a session by appending the following code:
...
The code above defines a `setSession` method that uses Xata SDK methods to insert a record in session table in your database. Define a method to update a session by appending the following code:
...
The code above defines an `updateSessionExpiration` method that uses Xata SQL query to update the `expires_at` attribute in a particular session in your database. Define a method to delete all expired sessions by appending the following code:
...
The code above defines a `deleteExpiredSessions` method that uses Xata SQL query to delete all the sessions whose `expires_at` attribute have expired. Define a method to obtain a session by appending the following code:
...
The code above defines a `getSession` method that uses Xata SDK method to retrieve a single matching record for a given session from your database. Define a method to obtain a user associated with a session by appending the following code:
...
The code above defines a `getUserFromSessionId` method that uses Xata SDK method to retrieve a single matching record for a given session from your database. If a session is found, it further queries to obtain the user associated  with the session. Otherwise, it returns `null`.
...
- Uses the `adapter` to create a new Lucia instance wherein the session cookie set to `Secure` if the application is running in the production environment.
- The `lucia` instance defines the `getUserAttributes` function to only fetch `paid` and `email` attributes from the user record(s).

To create type definitions for the user and session associated, append the following code:
...
- `DatabaseUserAttributes`: An object that would contain `email` attribute as a `string`, and `paid` attribute as `boolean` or `null`.
- `Lucia`: As the type of the lucia instance created.

### Create Utilities for User Authentication Status

To retrieve the authentication status of the session, you would need to obtain the user information associated with the request. Create a file `user.ts` with the following code:
...
- Exports `getUser` function which uses the `getSessionID` function to obtain the session id. Then, it uses Lucia's `validateSession` utility to obtain the user data information with the session.
...
The code above creates an sign up route in the Astro application containing a form that POSTs to `/api/sign/up` with user's email and password. Create a `src/pages/api/sign/up.ts` file with the following code to handle the form submission:
...
- Imports an utility from Lucia SDK to create random ID(s), the `getXataClient` utility to obtain the in-memory Xata instance, Argon2id class by `oslo` to hash given password(s).
- Exports a POST HTTP handler to only respond to incoming POST request(s).
...
The code above creates an sign in route in the Astro application containing a form that POSTs to `/api/sign/in` with user's email and password. Create a `src/pages/api/sign/in.ts` file with the following code to handle the form submission:
...
The code above creates an `/protected_and_paid` route in the Astro application that does the following:

- Checks if there's a user associated with the session and if they have paid.
- If the above is not true, it returns a 403 Unauthorized response. Otherwise, it returns a HTML page containing the paid and protected information.
...
The code above creates an `/protected` route in the Astro application that does the following:

- Checks if there's a user associated with the session.
- If the above is not true, it returns a 403 Unauthorized response. Otherwise, it returns a HTML page containing the protected information.
