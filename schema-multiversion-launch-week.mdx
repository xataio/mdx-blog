---
title: Schema multi-version early access preview goes live today
description: Schema multi-version, released today, addresses one of the most common pain points of application deployment - keeping your application code and database schema in sync. Starting today, database migrations in Xata can present two versions of your schema, old and new, to client applications allowing users to roll out new versions of their production applications with confidence.
image:
...
# Schema multi-version in Xata

The early access preview of schema multi-version in Xata allows you to start a migration from the Xata UI and have both versions of your database schema available to applications for the duration of the migration. When your application rollout is complete, the migration can be completed leaving you with just the final version of your database schema.

Let's take a look at how this works in practice.

First, we'll create a table in a fresh database, populate it with some data and then create a migration to add a `NOT NULL` constraint to a column in the table. We'll see how schema multi-version allows two versions of the database schema, one with the constraint and one without, to be made available to applications.

## Creating the schema

We'll use the Xata UI to:

- Create a simple `users` table with a `name` column
- Populate the table with some random data

In a new Xata database (with Postgres access enabled), we can create a new table in the Xata UI and populate it with some random data:

![Create a new `users` table in the Xata UI and add a `name` column](images/schema-multiversion-launch-week/new-table.png)
...
We can take this migration and run it via the Migration Editor in the Xata UI:
...
In order to be able to take advantage of schema multi-version, applications need to be able to connect to the correct version of the database schema. This is done using the Postgres [search_path setting](https://www.postgresql.org/docs/current/ddl-schemas.html#DDL-SCHEMAS-PATH). The `search_path` setting determines which schema will be searched for non-schema-qualified table accesses. By setting the `search_path` appropriately, client applications can access the version of the database schema that is compatible with the deployed application code.
...
Completing an in-progress migration can be done from various places in the Xata UI, including the migrations widget on the dashboard:

![The migration can be completed from the Xata UI](images/schema-multiversion-launch-week/complete.png)

Once completed, the table UI once again shows just one version of the `name` column in the `users` table:
...
Schema multi-version simplifies one of the most common pain points when deploying applications: keeping the deployed versions of your applications and the database schema in sync.

Xata's new schema multi-version feature allows you to deploy your applications and make database schema changes independently and with confidence.

We are excited to share this early access preview with you and look forward to your feedback.
