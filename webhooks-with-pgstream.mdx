...
curl localhost:9900/webhooks/subscribe \
    ...
    -d '{"url":"http://localhost:9910", "schema": "public", "table": "users"}'
```
...
Postgres values over the page size (commonly 8k, after compression) are stored separately from the main table data in [TOAST storage](https://www.postgresql.org/docs/current/storage-toast.html). This means they also not included in the logical replication events by default, unless they are changed by the current UPDATE statement. This is a gotcha: you are expecting all columns to always be present, but once they reach a certain size they will be missing.
...
curl localhost:9900/webhooks/subscribe \
  -H "content-type: application/json" \
  -d '{"url":"http://localhost:9910", "schema": "pgstream", "table": "schema_log"}'
```
...
Sending webhooks events from Postgres logical replication events is a non-trivial task, especially if you look at the corner cases and gotchas. However, [pgstream](https://github.com/xataio/pgstream) makes it pretty easy to get a nice, JSON-encoded, and consistent stream of events in the form of webhooks.
