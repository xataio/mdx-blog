...
You can find more examples in the [documentation](https://xata.io/docs/sdk/sql/overview).
...
In the above, note that the `Model` type adapts the Xata generated `DatabaseSchema` type to the `Model` type used by Kysely. This means that Kysley is aware of the tables and columns from Xata. For more details on how to use this integration, see the [documentation](https://xata.io/docs/integrations/kysley).

### Drizzle integration

We're also happy to announce an experimental integration with [Drizzle](https://github.com/drizzle-team/drizzle-orm). Drizzle is an TypeScript ORM that is focused on type-safety and that follows the SQL syntax as closely as possible. Drizzle's main philosophy is "If you know SQL, you know Drizzle ORM".
...
For now, we recommend manually defining the schema like in the example here. We are working on utility functions to automatically generate the Drizzle model from the Xata database schema.

## Test SQL statements in our playground

The Xata playground has also been extended to support SQL statements in addition to TypeScript code. Building off of the new SQL support, we've also added a few more enhancements to the playground. You can now tab through multiple sessions and name them to easily identify for re-use.
