...
We've just released `v0.9` of `pgroll`, our open-source schema migration tool for Postgres. As we've done for previous releases, we'll take a look at some of the headline features that made it into the release and look ahead to what's coming next. Previous installments in this series of release blogs can be read here:
...
It's also possible to specify exclude constraints using the table-level `constraints` field . Examples of how to do so can be found in the [documentation for the create_table operation](https://pgroll.com/docs/v0.9.0/operations/create_table).
