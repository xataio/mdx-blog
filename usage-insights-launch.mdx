...
- AI questions: The number of questions sent to the `ask` endpoint - an integration with OpenAI and ChatGPT
- Total branches: The current total number of of database branches

We are actively working on enhancements to the usage monitoring feature. We will shortly expose these metrics on a database branch level, and we plan to release further metric insights in time.
...
Additionally, pro plan workspaces receive a metrics summary for the current calendar month on the workspace landing page. This lets users get an up-to-date higher level overview of the workspace usage at a glance.
![Pro workspace overview](images/usage-metrics-launch/pro-usage-overview.png)

[Free plan](/docs/concepts/pricing#free-plan) workspaces also receive a glimpse into their usage on the workspace landing page. This concisely shows the percentage of available [free tier resources](/docs/concepts/pricing#pricing-plans) that have been used in the current calendar month.
...
Also, because we have chosen to use the [Merge Tree](https://clickhouse.com/docs/en/engines/table-engines/mergetree-family/mergetree) backend on Clickhouse it will occasionally merge snapshots together allowing it to further combine rows for the same worspace or branch together. This allows Clickhouse to effeciently store data that has a relativley low cardinality, but at the same time doesn't stop it from being able to also store high cardinaltiy data.
