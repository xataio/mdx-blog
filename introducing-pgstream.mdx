...
image:
  src: https://raw.githubusercontent.com/xataio/mdx-blog/main/images/pgstream/pgstream-banner@2x.png
  alt: pgstream
author: Esther Minano Sanz
authorEmail: esther@xata.io
date: 07-17-2024
tags: ['open-source', 'postgres', 'fpReplication', 'fpPgstream', 'fpPgroll', 'fpPgzx', 'oss']
published: false
slug: pgstream-postgres-replication-schema-changes
ogImage: https://raw.githubusercontent.com/xataio/mdx-blog/main/images/pgstream/pgstream-banner@2x.png
---
...
- **Schema change tracking and replication of DDL changes:** it’s no surprise, since this was one of our biggest requirements, that it made it as an integral feature of pgstream. We will go into a bit more detail on how this is implemented below.
- **Modular deployment configuration:** pgstream modular implementation allows it to be configured for simple use cases, removing unnecessary complexity and deployment challenges - the only hard requirement for pgstream is a Postgres database! However, it can also easily integrate with Kafka for more complex usecases.
...
![pgstream_arch_v1.png](https://raw.githubusercontent.com/xataio/mdx-blog/main/images/pgstream/pgstream_arch_v1.png)

### Tracking schema changes

One of the main differentiators of pgstream is the fact that it tracks and replicates schema changes automatically. How? It relies on SQL triggers that will populate a Postgres table (`pgstream.schema_log`) containing a history log of all DDL changes for a given schema. Whenever a schema change occurs, this trigger creates a new row in the schema log table with the schema encoded as a JSON value. This table tracks all the schema changes, forming a linearised change log that is then parsed and used within the pgstream pipeline to identify modifications and push the relevant changes downstream.

![schema_changes_ddl.png](https://raw.githubusercontent.com/xataio/mdx-blog/main/images/pgstream/schema_changes_ddl.png)
...
This is only the beginning! We plan to continue developing pgstream and exploring how it can make it easier to replicate data. We are looking forward to your feedback! 
