...
pgzx is our open-source framwork for developing PostgreSQL extensoins in Zig.
...
- Function Manager API
  - optional errors
  - PG_EXPORT
  - example extension sqlfns

# Highlights

We have adde quite a number of improvements and fixes since the first update.
Unfortunately this includes some breaking changes. Let's go through a number of 
...
not focus on a specific release version, but try to follow the latest Zig
...
use the projects develoment shell when testing the example extensions provided,
...
In this release we've added some improvements to the Function Manager API,
which is implemented in the `pgzx.fmgr` module.
...
[Go](https://go.dev/) community as "Functional Options". In essence in the
functional options pattern a function accepts a variable number of arguments
where each argument implements a common interface. This can be a function or an
actual Go interface. The receiver might establishe an internal 'context' and
calls each passed argument with the given context. Once all options have
updated the 'context', it is used as configuration of the function body. The
receiver may also choose if it even needs to evaluate the options passed.
...
Once our `ereport`  is compiled it behaves exactly like the C macros (minus us using the zig string formatting functionality).
...
The SPI interface requires you to connect and finish to the SPI subsystem. Internally SPI will maintain a stack of "SPI frames". Often times you will find yourself writing:
...
We currently do not include all headers available, but rather selectively add new headers when needed.
If you are missing a header, please open a PR or an issue on the [pgzx Github
repository](https://github.com/xataio/pgzx).

The full set of headers from the `foreign`, `nodes`, `optimizer`, and `catalog` folder have been added.
