...
Since we released the [Xata Agent](https://github.com/xataio/agent), our Postgres AI expert that monitors and proposes improvements for your PostgreSQL instances, one of the most common questions we’ve got is a variation of:

> I’m currently using \{Datadog|Grafana|Elastic\} for monitoring Postgres, how is this tool different?

We usually answer this in terms of what the Xata Agent already does today for Postgres, but I think the question gets even more interesting if we’re generalizing to: **what can LLMs bring to the observability space?**
...
From what we’ve seen running the Xata Agent so far, the cost of current LLMs is still much smaller (like 2 orders of magnitude) compared to what we pay our observability solution. An this is with the most expensive models from OpenAI and Anthropic.
