---
 title: 'Indexing Files with the Python SDK'
 description: 'A walkthrough on how to extract and index text content from file attachments using the Python SDK.'
 image:
   src: https://raw.githubusercontent.com/xataio/mdx-blog/main/images/TBD_blog_illustration.jpeg
   alt: 'Extracting and indexing text from file attachments with Python'
...
Xata’s [file attachments](https://xata.io/docs/concepts/file-attachments) is a powerful feature that enables you to store files in your database, alongside other common [column data types](https://xata.io/docs/concepts/data-model#column-types).
...
The actual file content is not *indexed* by default. By “indexed”, we are referring to the process of extracting the file’s text content and storing it in a text column where it can be accessed, searched and queried.

So how can we use Xata’s advanced features such as [Free-text Search](https://xata.io/docs/sdk/search) and [Ask AI](https://xata.io/docs/sdk/ask) on our file attachment’s actual content? 

In this article we are exploring quick and easy ways for indexing file attachments while taking a look into Xata’s Python SDK which is now [Generally Available](https://xata.io/blog/announcing-the-python-sdk-ga)!

## Summary

The *xfileindex* script in the [xtools](https://github.com/xataio/xtools) repo can be used to extract content from text or pdf file attachments stored in your Xata records and index it as raw text, which enables us to use the content with Xata’s Query, Free-text search and Ask AI capabilities.

The [readme](https://github.com/xataio/xtools/blob/main/xfileindex/README.md) file provides usage examples and the full list of parameters.

## Using Xata’s Python SDK

Python is a beginner-friendly programming language which comes pre-installed with many popular OS distributions but can also be [downloaded](https://www.python.org/downloads/) and installed on any platform. `pip` is the package installer of Python and it almost always exists alongside Python wherever it is installed (but there also more ways to [install pip](https://pip.pypa.io/en/stable/installation/) in any case). Pip fetches modules from the official Python Package Index (PyPI), where you will find the [Xata Python SDK](https://pypi.org/project/xata/) package. It can be installed with the command `pip install xata`.

Once the Xata Python SDK has been installed, we can import the Xata Client in our code (`from xata.client import XataClient`) and use it to interact with our Xata database.

Note that the version of python in your system (`python --version`) should be 3.8 or higher to use the [script](https://github.com/xataio/xtools/tree/xfileindex-v0.0.1/xfileindex).

## A Python script to index file attachments

Now that we’ve covered how to use Xata with Python, let’s jump straight to the point of indexing file attachments.

We will assume that you’ve already stored some files in your database under columns of `file` or `file[]` type. Check out the [documentation](https://xata.io/docs/concepts/file-attachments) for more information on these column types.

The [script](https://github.com/xataio/xtools/tree/xfileindex-v0.0.1/xfileindex) scrolls through our Xata database, looks for `file` or `file[]` columns with text, csv or pdf content (*mediaType*), extracts the text and writes it into text columns in a target table. Which is all we need, in order to put the file content to work with Query, Search, Ask AI features and whatever else is to come!

Here are the essential steps to run this script:
...
“Atomic” refers to writing one record, a single chunk of text, with each HTTP/S request to Xata.
...
Once we reach the maximum number of operations that can run as a single transaction, specified by the `tsize` script parameter, we call the `trx.run()` method. The response code is checked for retriable status code `429` which is returned in case of hitting [rate limits](https://xata.io/docs/rest-api/limits#rate-limits).
