...
First, let’s click **+ Add column** and select `File`.
...
In the `profiles` table, we want to store the attributes such as user’s unique slug (the path of the url where gallery will be displayed of that user), their name, their image with it’s dimension, and it’s transformed image’s base64 hash. We’ll reap the benefits of storing the hash to create literally 0 Cumulative Layout Shift (CLS) page(s).
...
You can also allow transitions on form submissions by adding ViewTransitions.
