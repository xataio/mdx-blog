...
description: "Let's look at the numbers behind our free tier. How much does a database costs us and why are we giving them for free."
image:
...
With high-availability included and fully managed and monitored by us, you can count on it just like you count on an online Google spreadsheet or any other SaaS product. But it is a powerful Postgres database, and comes with 15 GB of storage for free.

When we tell people about this, we're often met with disbelief, quite understandably. So let's see how we do it and if it's going to bankrupt us.

## Why even have a free tier?

To state the obvious, the goal of a free tier is to ultimately generate paying customers. The cost of the free tier is mostly justifiable as part of the cost of finding new customers, similar to e.g. marketing expenses.

A typical funnel looks something like this:

![Free tier funnel](images/freetier-funnel.png)

At the high level, from the new users that enter the funnel, some become inactive soon after. That can be because the product didn't meet their needs or that they were simply looking around without an actual project to work on. In order for a free tier to be financially reasonable, given enough databases, it's essential that the cost per "inactive database" is minimal.

Other group of users are developing their application or project. If that application reaches production, it's likely that they will stay on the platform for a long time.

From those, a part of them will convert to paying customers. We generally find that teams (even small non-profits) that go into production want to switch to a paid plan, where they can get dedicated resources and the best support. But there is also a category of individual developers that want a free and reliable database service for their personal side projects. While these individual builders don't always convert to paying users, they do often help by by recommending us at work, by spreading the news, or simply by giving us feedback.
...
In addition, we pair this Postgres clusters with an OpenSearch clusters to which we automatically replicate the data. This is because we want our free tier to demonstrate not only our Postgres service, but also "database platform" additions, and painful replication to a search/aggregations engine is one of them.

![Postgres + OpenSearch cluster](images/freetier-cell.png)

That's right, the free plan includes not only a Postgres free tier, but also a search engine free tier based on OpenSearch.

The main issue with using shared clusters is the "noisy neighbor" issue. This happens when a free users generates enough load to negatively affect the other users in the same clusters.
...
If you're used cloud DBaaS pricing, you might have noticed that I omitted something: the above only talks about compute cost, not the storage cost. Storage costs us $0.10 / GB-month, and we offer 15 GB in the free tier, so that can cost us up to $1.5, even for inactive DBs. However, in practice, free inactive databases typically don't load that much data and, in average, the storage cost is much smaller than that.
...
In all of these cases, creating databases (or branches) instantly via a simple API call and have them cost next to nothing at small scale comes in really handy. The ability to move them from one cluster to another, without downtime, makes it easier to optimize your underlying instances.

## Conclusion

We went through the unit economics for our free tier. The shared clusters approach, combined with the ability to move logical databases between physical clusters, gives us the flexibility to group databases based on their activity level, mitigating the noisy neighbor problem. While this tech helps us offer a sustainable free tier, it is also a fit for larger use cases.

Want to check it out and increase our bill? Simply [sign up](https://app.xata.io/), it's on us 😅
