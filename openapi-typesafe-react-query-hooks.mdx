...
description: "The Xata engineering team values predictability at all levels of our codebase. This blog post is about how we handle API communications in a predictable way."
image:
...
To us at Xata engineering, we consider spending time reverse engineering APIs just to know what kind of data is available to be neither fun nor valuable. Instead, through [OpenAPI](https://www.openapis.org/) we get some insights into the possibilities exposed by some APIs. However, why stop there? Reading an OpenAPI spec and manually writing code is… let's say, also not our thing.
...
Given these constraints, we saw potential for innovation. _We had to do something!_ And this is why we started to craft [openapi-codegen](https://github.com/fabien0102/openapi-codegen).

The goals were clear:

1. Generate human-readable types, so we can review TypesScript files instead of a large OpenAPI YAML file.
1. Embed all possible documentation, so we can have access to the documentation directly through [IntelliSense](https://code.visualstudio.com/docs/editor/intellisense#:~:text=IntelliSense%20is%20a%20general%20term,%2C%20and%20%22code%20hinting.%22).
1. Generate ready-to-use React Hooks, with `url` and `types` baked into the component to avoid any human mistakes.
1. Be as flexible as possible: so far we can generate fetchers & react-query hooks, but let's not restrict the possibilities.
...
How is this working? Let's have a look of the generated `useSearchUsers` to understand.
