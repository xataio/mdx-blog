...
We could not claim to simplify working with files and then ask customers to configure and manage their own CDN to get reasonable global performance. Hence, the file attachments support comes with CDN included. There is no opt-in and or configuration needed; all direct access URLs are served through a CDN by default. In essence, all files retrieved through a URL are cached at the edge, making the following requests blazing fast.
