...
image:
  ...
  alt: 'Introducing file attachments, scalable storage embedded in your database'
author: Sorin Toma & Alex Francoeur
date: 08-30-2023
published: false
...
Both files and relational data are ubiquitous in today’s applications. Even the most basic scenarios generally require both structured, relational data and file storage. Think about a blog which uses both posts metadata and images, think about a product catalogue or a document library. They all require basic data record management, query capabilities, but also storage and access to large binary objects like images, videos, documents.
...
Whether you’re building a full scale e-commerce application, a personal blog or evolving your product to be AI-native — file attachments unlock a number of new use cases, provides an opportunity to simplify your architecture and reduce the number of services you rely on.
...
Let’s have a deeper look into what gets simplified and how does Xata achieves this.

## Attachments, not buckets

We’re really excited about todays release because it packs a lot of functionality into the simple experience of adding another column to your database. We wanted this experience to feel familiar, like adding and viewing an attachment in a spreadsheet instead of dropping a file into a generic bucket. As we started to work on the file attachments, we had the following goals in mind:
...
The first step in our design was to acknowledge that relational data and large binary objects have very different consumption models and trying to fit both into the same storage service would lead to unacceptable compromises. One size doesn’t fit all. Very briefly, serving large binary objects from a relational database cannot match the performance of a dedicated storage service in terms of compute cost, concurrency and throughput. The opposite is even more obvious, a storage service cannot match the querying and data management capabilities of a relational database.

As with any interesting designs, we had to work out a solution to address apparent contradictory requirements, the APIs for relational data and binary objects should be unified, while the backend storage must be different to meet the expected performance and feature set.

### Introducing the file column type

At API and database schema level, the binary object data type became the `file` column type. You can attach one or multiple files to a record in your database. This approach allowed all existing Xata features to work with the file type without any API change.

![Adding a file column type](/videos/fa_add_file.mp4)

Using the existing Xata APIs, either REST or Xata SDKs, using the same connection, a developer can now upload a file, download a file, run queries over files using filters, aggregations, joins, and even run search queries to match file metadata.
...
![Making a file public](/videos/fa_make_file_public.mp4)
...
We have launched this functionality with an example image gallery app to highlight the developer experience you get with Xata and give you a starting point to try it out. This example combines full text search, aggregations, files and image transformations. If you’d like get started, be sure to check out our [example repository](https://github.com/xataio/sample-nextjs-chakra-gallery-app).
