---
title: "Unveiling the impact of REPLICA IDENTITY FULL on PostgreSQL's performance"
...
At [Xata](https://xata.io/), we've come to rely on the magic of Postgres logical replication 
events. Why? Because they provide us with a dependable stream of information about
every single change that occurs within our database. This stream of data is incredibly 
valuable in various situations, such as when we need to synchronize data with Elasticsearch, use web-hooks, or even tidy up attachments.

Now, let's dive into one important aspect of logical replication: the "replica identity." 
...
This configuration parameter plays a crucial role in shaping the replication process, allowing us to fine-tune and optimize our data synchronization.

You’d typically use the primary key or an index as the “replica 
identity” and avoid setting the “replica identity” to full. This recommendation is supported by the following quote from the [Postgres docs](https://www.postgresql.org/docs/current/logical-replication-publication.html):
...
While the recommendation to avoid setting the "replica identity" to full may seem discouraging at first, there are valid reasons to explore its potential benefits. In this blog, we'll delve deeper into why enabling it could be advantageous. By examining the inner workings and performance implications, this blog post aims to provide a comprehensive analysis of the impact of enabling `REPLICA IDENTITY FULL`.
...
In our specific case, we uncovered two compelling reasons that convinced us to consider enabling `REPLICA IDENTITY FULL`:

* **Complete Before and After Values**: By default, when performing `UPDATE` and `DELETE` in Postgres, only the old values of the "identity columns" are included. However, if you require both the values before the change and the values after the change, setting the replica identity to "full" tricks Postgres into providing the old values as well.

* **Ensuring Inclusion of TOASTed Columns**: The UPDATE WAL events in Postgres do not include [TOASTed columns](https://www.postgresql.org/docs/current/storage-toast.html), unless those columns have been modified in the current operation. In practical terms, this means that if you have column values larger than 8KB, they will be excluded from the update events. However, by setting the replica identity to `FULL`, the TOASTed columns are always included in the Write-Ahead Log (WAL) events.

To illustrate the above, let’s look at an example using [wal2json](https://github.com/eulerto/wal2json). An update event looks something similar to the following:
...
The table, however, contains an additional column called `description`. Unfortunately, this column is entirely excluded from the event. Why? The reason is that the value of the `description` column is large and stored as a TOASTed value. Unless the `UPDATE` command modifies the `description` column directly, it is entirely skipped and not included in the `columnvalues`.
...
- The `oldkeys` section includes all the columns and their respective values **before the update**, including the TOAST value for the `description` column.

It is worth noting that by setting the replica identity to the primary key or another unique index, you can retrieve sufficient data from the replication stream to fully synchronize a Postgres replica. This approach allows you to overcome the mentioned limitations by managing states external to Postgres. It is important to acknowledge that this method may require substantial effort in terms of resource allocation and additional code compared to a simple `ALTER TABLE` statement.

# Impact on the replica and changes in Postgres 16

The quote mentioned at the beginning of the blog post, containing the phrase "very inefficient," was originally sourced from the currently stable Postgres 15 documentation. However, it is important to note that this wording has been altered in the currently under-development [Postgres 16 docs](https://www.postgresql.org/docs/16/logical-replication-publication.html):
...
The above suggests that the term "very inefficient" relates to the process where the subscriber identifies the rows that have been updated or deleted. It's great top know this is being improved in version 16 of Postgres. In the upcoming version, the subscriber will be able to use indexed columns even if they are not explicitly set in the replica identity. This improvement aims to streamline the identification process and enhance overall efficiency.
...
While the worst potential impact of `REPLICA IDENTITY FULL` is on the replica, as we’ve seen above, it does also impact the primary because it requires more data to be stored in the WAL and it causes it to send more data over the network to the subscribers. This leads to increased consumption of IO bandwidth, higher CPU usage, and greater use of RAM resources.

How much more? The extent of the increase depends on the nature of your write traffic, the quantity of large values present, and the frequency of updates to those values.

Let’s look at it by the event type:

### Inserts

Inserts are unaffected, because they don't involve “old values” and the large values are included regardless of the `REPLICA IDENTITY` setting.
...
In conclusion, the size of the WAL events for updates is nearly doubled if there are no TOASTed values, and potentially way more than doubled if there are TOASTed values. 

### Deletes

For `DELETE`, normally only the key is logged in the WAL, and by enabling `REPLICA IDENTITY FULL`, all columns become part of the key, meaning that the whole row is logged and sent over the wire on delete.

# Benchmarking

We guessed that the cost of `REPLICA IDENTITY FULL` would be worth it in our case, so decided to put it to test! We wanted to simulate something close to the worst case scenario (but still realistic), so we have extended our benchmark test suite to have a test with a lot of large values and performing a lot of updates. The test didn’t push the resources to the limit but was heavy enough for us to measure the impact.
...
If you’re using logical replication between instances, the impact from `REPLICA IDENTITY FULL` will likely be manageable as long as the replicated tables have a primary Key. If you are already on Postgres 16, not having a primary key but another unique index might also be okay.
