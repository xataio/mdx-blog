---
title: "On the performance of REPLICA IDENTITY FULL in Postgres"
description: "The blog post analyses the performance impact of REPLICA IDENTITY FULL by looking at the source code and performing benchmarks."
image:
...
At [Xata](https://xata.io/), we’re relying on the Postgres logical replication
events quite a bit. Having a reliable stream with everything that has changed
in the database is really useful in a number of cases, including data syncing
to Elasticsearch, web-hooks, attachments clean-up, and more. One of the
configuration parameters for logical replication is the so called “replica
identity”:
...
You’d typically want to use the primary key or an index as the “replica 
...
- UPDATE WAL events don’t include [TOASTed columns](https://www.postgresql.org/docs/current/storage-toast.html), unless that column has changed in the current operation. In practice this means that if you have column values over 8KB, you will miss them from the update events. Again, by setting the replica identity to FULL, we make sure they are always in the WAL event.
