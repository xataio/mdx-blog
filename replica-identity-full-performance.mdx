...
In our specific case, we discovered two compelling reasons that convinced us to consider enabling `REPLICA IDENTITY FULL`:
...
The quote from the beginning of the  blog post with the “very inefficient” wording is actually from the — currently stable — Postgres 15 docs , but it was changed in the [Postgres 16 docs](https://www.postgresql.org/docs/16/logical-replication-publication.html) — currently under development:
...
If you find the above interesting and want to work on similar challenges, make sure to check out the [Xata careers](https://xata.io/careers) page.
