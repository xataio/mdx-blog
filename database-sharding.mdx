...
<Expand title="Hash-based sharding">
  | Aspect | Description |
  |----------------|------------------------------------------------------------------------------------------------------|
  | **How it works** | A hash function is applied to a key attribute, and the result determines which shard the record
  goes to. | | **Use case** | Good for evenly distributing data, especially when the range or list is not predictable. |
  | **Example** | User IDs are hashed, and the hash value determines the shard. |
</Expand>

<Expand title="Directory-based sharding">
  | Aspect | Description | |----------------|-------------| | **How it works** | A separate lookup service or directory
  maps each data item to a specific shard. | | **Use case** | Offers flexibility in sharding and can handle more complex
  data distribution scenarios. | | **Example** | A mapping service directs user queries to the appropriate shard based
  on the user's location or preferences. |
</Expand>

<Expand title="Geo-sharding">
  | Aspect | Description | |----------------|--------------------------------------------------------------------------|
  | **How it works** | Data is sharded based on geographical locations. | | **Use case** | Useful for services that need
  to reduce latency by locating data closer to the user. | | **Example** | Social media posts might be stored in a shard
  located in the same region as the user. |
</Expand>

<Expand title="Key-based sharding">
  | Aspect | Description |
  |----------------|------------------------------------------------------------------------------------------------------------|
  | **How it works** | Also known as customer sharding. The sharding is done based on a specific key, such as customer
  ID or tenant ID. | | **Use case** | Effective for multi-tenant applications where data isolation between tenants is
  crucial. | | **Example** | Each comic's data is stored in a separate shard. |
</Expand>
...
<Expand title="Functional sharding">
  ...
  different types of data have vastly different access patterns. | | **Example** | One shard handles all the
  transactional data, while another handles logging or archival data. |
</Expand>
