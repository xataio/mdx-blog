...
<Expand title="Directory-based sharding">
  * **How it works**: A separate lookup service or directory maps each data item to a specific shard. - **Use case**:
  Offers flexibility in sharding and can handle more complex data distribution scenarios. - **Example**: A mapping
  service directs user queries to the appropriate shard based on the user's location or preferences.
</Expand>
...
<Expand title="Geo-sharding">
  * **How it works**: Data is sharded based on geographical locations. - **Use case**: Useful for services that need to
  reduce latency by locating data closer to the user. - **Example**: Social media posts might be stored in a shard
