...
Recently, we have started on a journey to build the tooling companies need around Postgres to easily build, manage and scale their applications. Our goal is to create a series of open source projects where each aims to solve a pain point users have with Postgres. The first open source tool, [pgroll](https://github.com/xataio/pgroll), provides zero-downtime and reversible schema migrations for any PostgreSQL database. This has also generated [quite](https://news.ycombinator.com/item?id=37752366) a bit of [interest](https://simonwillison.net/2024/Jan/30/pgroll/) from the broader Postgres community. Later this week we'll be announcing another open source project that we think you'll love 🦎.
...
Finally, we have integrated [pgroll]() in our platform. pgroll is our open-source project for zero-downtime, reversible schema migrations. While `pgroll` is a CLI tool, in order to integrated it seamlessly in the Xata platform, we have developed a server API for it, which integrates with our control plane. For now, this allows us to keep track of all DDL statements that are being executed and opens the door to a much better schema-changes workflow. Look out for a detailed blog post this week about pgroll and how we integrated it into Xata.

## Get started

The Xata Postgres service is currently in public Beta. You can opt-in to the Beta by flipping a flag in your [workspace's settings](). Once you have enabled the flag, when creating a new database, you can select whether you want to have Postgres direct access or not. Because the underlying changes are significant, at the moment, it's not possible to migrate existing Xata database to the Postgres kind. However, we are planning to provide a migration path during the Beta period.
