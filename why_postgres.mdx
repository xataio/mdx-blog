...
description: "We're announcing the next major evolution for the Xata data platform: we are now offering a serverless Postgres service, and we think it is one of the best ways to run Postgres in the cloud today."
image:
...
If you haven't heard about Xata before, let's start with some introductions: we have [launched](https://xata.io/blog/xata-public-release) the Xata database service in November 2022 with a product that aimed to provide the best possible developer experience for a database. We felt back then that databases have been left behind a little when it comes to modern developer workflows, APIs, and experience in general. So we re-thought things from the ground up, without tying ourselves to any existing database technology.

We imagined our product as the middle ground between Airtable and traditional databases, and we think we've done a pretty good job at that. Like in Airtable, the "xatabases" start instantly, have rich types, have no cold starts, and are never paused. Like in traditional databases, they have no rows limitations, have ACID transactions, constraints and referential integrity, and are highly performant.
...
First, the Xata architecture was built from the start to be able to offer a free tier sustainably, thanks to the use of shared clusters and auto-scaling. Currently, a free **active** database with constant traffic within the rate limits costs us less than $1.5/month. That includes not only Postgres but also Elasticsearch and the replication between them, as well as running a replica for high-availability. That's why we don't really need to pause or sleep databases in case of inactivity yet, at least not in a matter of days. We are planning, however, to pause the replication if it is not used and we will eventually off-load databases after a long period of inactivity, measured in months, to further improve the economics of our free tier.
