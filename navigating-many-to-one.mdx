...
![Xata schema showing a many-to-one relationship](images/many-to-one-relationships.png)
...
- Subqueries: the main advantage is that we don't increment the number of queries at the cost of adding some complexity to the existing one (and to the statement builder code)
- JOINs: we would also keep the number of queries the same, but the code could get really complex and we would need to filter out lots of duplicated data in each row
- N+1 queries: we would keep our SQL simple at the cost of (still) making N+1 queries to the database and joining the responses together. This would still be an improvement because the (backend - PostgreSQL) latency is an order of magnitude less than the (client - backend) latency

So far, we went on with the subquery approach because it was simple and easy to adapt in the future, if needed. To make it even more simple
we take advantage of the [json_agg()](https://www.postgresql.org/docs/current/functions-aggregate.html) function to get a JSON document with all the related data ready to be returned in the response.
