...
description: "Schema migrations are a pain. Let's review the problem space and tools available to make reversible schema changes in production with zero-downtime."
image:
...
This pattern comes from fear of breaking the application, which is fair. Though at the end up the day, having a table with hundreds of columns can lead to a few issues. You can encounter performance issues when executing certain queries, maintain compatibility code in your application for an extended period, and create confusion for anyone new to the database
...
Having data consistency between the changes, avoiding schema drift and backfilling the appropriate data correctly are some of the complexities that can lead to downtime. That’s why the more complex the application, the more complex the deployment process ends up being for database changes.
...
[Sqitch](https://sqitch.org/) is interesting because it’s positioned itself as a standalone change management system. It is unopinionated about your application framework, database engine or development environment. Unlike liquibase or flyway, there is no need to number or name your migrations a certain way. Sqitch handles this for you. It’s a unique solution that aims to make it easy to integrate into your existing environment.
